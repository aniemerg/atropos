...
class GAIABenchmarkConfig(BaseEnvConfig):
    ...
    dataset_path: str = Field(
        default="data/gaia", 
        description="Path to the GAIA benchmark data"
    )
    split: str = Field(
        default="validation", 
        description="Dataset split to use (validation, test)"
    )
    use_chat_completion: bool = Field(
        default=False,
        description="Whether to use chat completion API"
    )
    max_steps: int = Field(
        default=12,
            ...
            self.examples = [
                {
                    ...
                    "task": example["Level"],
                    "task_id": example["task_id"] if "task_id" in example else f"task_{i}",
                    "file_name": f"{self.config.dataset_path}/{self.config.split}/{example['file_name']}" 
                        if example["file_name"] else ""
                }
                for i, example in enumerate(self.dataset)
            ]
            
            logging.info(f"Loaded {len(self.examples)} examples from GAIA {self.config.split} set")
