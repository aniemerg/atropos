...
class AtroposServerModel(Model):
    ...
    def generate(
        ...
        **kwargs,
        ...
        try:
            # Create async method to handle the completion
            async def call_server():
                if self.use_chat_completion:
                    resp = await self.server.chat_completion(**completion_kwargs)
                    return resp, resp.choices[0].message.content
                else:
                    resp = await self.server.completion(**completion_kwargs)
                    return resp, resp.choices[0].text
            
            # Better approach to handle both sync and async contexts
            try:
                # Check if we're in an event loop
                loop = asyncio.get_running_loop()
                
                # We're in an async context, so we'll need to await directly
                response, content = asyncio.run_coroutine_threadsafe(
                    call_server(), loop
                ).result(timeout=completion_kwargs.get("timeout", 120))
            except RuntimeError:
                # We're not in an event loop, so create one
                response, content = asyncio.run(call_server())
