...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Run a GAIA benchmark task with OpenAI"
    )
    ...
    parser.add_argument(
        "--file-path", type=str, help="Optional file path to include with the task"
    )
...
def create_tools(file_path: Optional[str] = None) -> List[Tool]:
    ...
    tools.append(
        Tool(
            ...
            inputs={
                "path": {"type": "string", "description": "Path to file (optional)"}
            },
            function=read_file,
        )
    )
...
def run_task(args, prompt, file_path=None):
    ...
    model = OpenAIModel(model_id=args.model, api_key=args.api_key)
...
def main():
    ...
    if not args.api_key:
        print(
            "Error: OpenAI API key is required. Set it using --api-key or OPENAI_API_KEY in .env file"
        )
        return
    ...
    if args.is_file:
        ...
        try:
            ...
            for example in dataset:
                ...
                # If task not found
                raise ValueError(
                    f"Task ID {args.task_id} not found in the validation split"
                )
        except ImportError:
